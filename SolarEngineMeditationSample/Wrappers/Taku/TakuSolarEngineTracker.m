...
typedef NS_ENUM(NSInteger, TakuNetworkID) {
    Meta = 1,
    Admob = 2,
    Inmobi = 3,
    Applovin = 5,
...
@implementation TakuSolarEngineTracker
...
+ (NSString *)networkNameWithFirmID:(NSString *)network_firm_id{
    ...
    if (network_firm_id_int == Meta) {
        ...
        NSAssert(false, @"unstated network_id, you should state it here");
    }
    return adNetworkPlatform;
}

+ (void)trackAdImpressionWithAdType:(TakuAdType)adType extra:(NSDictionary *)extra {
    [TakuLogUtils i:[NSString stringWithFormat:@"TakuSolarEngineTracker.trackAdImpressionWithAdType: %ld, extra: %@", (long)adType, extra]];
    ...
    attribute.mediationPlatform = @"Taku";
