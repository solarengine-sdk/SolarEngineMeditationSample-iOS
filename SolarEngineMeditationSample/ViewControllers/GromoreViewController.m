...
@interface GromoreViewController ()

@property (nonatomic, strong) GromoreSampleAdManager *gromoreManager;
...
@implementation GromoreViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setupUI];
    [self setupGromoreManager];
}

- (void)setupUI {
    self.title = @"Gromore Ads";
...
- (void)setupGromoreManager {
    self.gromoreManager = [[GromoreSampleAdManager alloc] init];
    [self.gromoreManager initializeSDK];
}
...
- (void)loadInterstitial {
    [LogUtils i:@"Loading Gromore Interstitial ad"];
    [self.gromoreManager loadInterstitialAd];
}

- (void)showInterstitial {
    [LogUtils i:@"Showing Gromore Interstitial ad"];
    [self.gromoreManager showInterstitialAdFromViewController:self];
}

- (void)loadRewarded {
    [LogUtils i:@"Loading Gromore Rewarded ad"];
    [self.gromoreManager loadRewardedAd];
}

- (void)showRewarded {
    [LogUtils i:@"Showing Gromore Rewarded ad"];
    [self.gromoreManager showRewardedAdFromViewController:self];
}

- (void)loadBanner {
    [LogUtils i:@"Loading Gromore Banner ad"];
    [self.gromoreManager loadBannerAd:self];
    BUNativeExpressBannerView *bannerView = [self.gromoreManager getBannerView];
